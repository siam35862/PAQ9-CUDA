...
#define base_memory_level                                                      \
  19 // default base memory level, MEM=1<<base_memory_level+memory_level
#define GPU_VRAM_LEVEL                                                         \
  9 // perchantage of VRAM , default 5 means 50% of VRAM will be used for
    // compression
int memory_level =
    1;            // default memory level MEM=1<<base_memory_level+memory_level;
...
__device__ StateMap::~StateMap() { prediction_table = 0; }
...
public:
  ...
  __device__ int context4() { return hash2; }
  __device__ int context8() { return hash1; }
...
__device__ int Predictor::predict_next_bit() {
  ...
  if ((bc & 3) == 0) { // nibble boundary? update context pointers
    int pcr = pc & -r;
    U32 c4p = c4 << 8;

    if (bc == 0) { // byte boundary? update order-1 context pointers (cheap,
                   // lanes 0-3)
      if (lane == 0)
    ...
    if (lane == 10)
      cp[10] = hashtable->operator[](
          (lzp[chunk]->word1 * 7 + lzp[chunk]->word0 * 11 + c0 + pcr * 37));
  ...
  if (lane == 0) {
    sp[0] = &cp[0][c0];
    stretched_cache[0] =
        stretch->operator()(statemap[0]->predict_next_bit(*sp[0]));
  } else if (lane < N) {
    sp[lane] = &cp[lane][lane < 4 ? c0 : nibble];
    int st = *sp[lane];
    stretched_cache[lane] =
        stretch->operator()(statemap[lane]->predict_next_bit(st));
  }
  ...
  __device__ int code(int y = 0) {
    ...
    if (lane == 0) {
      while (((x1 ^ x2) & 0xff000000) ==
             0) { // pass equal leading bytes of range
        if (mode == COMPRESS)
          buffer[csize++] = x2 >> 24;
        x1 <<= 8;
        x2 = (x2 << 8) + 255;
...
__device__ Encoder::~Encoder() { buffer = 0; }
...
__global__ void paq9_cuda(size_t *input_size, char **input, size_t *output_size,
  ...
  if (mode == COMPRESS) {
    ...
    while (true) {
      ...
      int cp =
          lzp[chunk]
              ->predict_char(); // cheap, read-only; fine for every lane to call
      if (ch == cp) {
    ...
    if (input[chunk][0] == '1') {
      ...
      while (true) {
        ...
        Encoder encoder(mode, input[chunk], buffer[chunk], input_size[chunk],
                        itr);
...
static void computeWarpLaunchGeometry(int num_chunks, int &blocks,
                                      int &threadsPerBlock) {
  threadsPerBlock =
      MAX_THREADS_PER_BLOCK; // 256 => 8 warps/block, multiple of 32
...
void compress(char *destination_file, char *source_file) {
  ...
  for (int call_count = 0; call_count < device_call_count; call_count++) {
    ...
    // init_start_time); std::cout << "Device initialization time: " <<
    // init_duration.count() << " ms" << endl;
    cudaError_t err1;
    err1 = cudaGetLastError();
    if (err1 != cudaSuccess) {
    ...
    ////////////////////paq9_cuda call (warp-cooperative
    ///geometry)////////////////////////////
    // auto kernel_start_time = std::chrono::high_resolution_clock::now();
    // std::cout << "input size: " << input_size[0] << " Byte, Current chunks: "
    // << num_of_current_thread << endl;

    int blocks, threads;
    computeWarpLaunchGeometry(num_of_current_thread, blocks, threads);
    // std::cout << "Warp-cooperative launch: " << blocks << " blocks x " <<
    // threads
    //           << " threads (" << (32 * num_of_current_thread) << " raw
    //           threads for "
    ...
    // std::chrono::duration_cast<std::chrono::milliseconds>(kernel_end_time -
    // kernel_start_time);
...
void decompress(const char *destination_file, const char *source_file) {
  ...
  if (mode == 's') {
    ...
    for (int i = 0; i < num_of_thread; i++) {
      cudaMallocTracked(&temp_d_input[i], (chunk_B + 2) * sizeof(char));
    ...
    for (int call_count = 0; call_count < device_call_count; call_count++) {
      ...
      // auto kernel_start_time = std::chrono::high_resolution_clock::now();
      paq9_cuda<<<blocks, threads>>>(
          d_input_size, d_input, d_output_size, d_output, encoder_buffer,
      ...
      // auto kernel_duration =
      // std::chrono::duration_cast<std::chrono::milliseconds>(kernel_end_time -
      // kernel_start_time); std::cout << "Kernel execution time: " <<
      // kernel_duration.count() << " ms" << endl;
    ...
    std::cout << "Total Execution Time: " << elapsed_time2.count() << " seconds"
              << endl;
