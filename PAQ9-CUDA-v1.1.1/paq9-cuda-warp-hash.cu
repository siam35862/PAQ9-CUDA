...
__device__ int Predictor::predict_next_bit() {
    ...
    if ((bc & 3) == 0) { // nibble boundary? update context pointers
        ...
        if (bc == 0) { // byte boundary? update order-1 context pointers (cheap, lanes 0-3)
            if (lane == 0)
        ...
        if (lane == 10)
            cp[10] = hashtable->lookup_partitioned((lzp[chunk]->word1 * 7 + lzp[chunk]->word0 * 11 + c0 + pcr * 37),
                                                   lane - 4, NUM_HT_PARTITIONS);
...
__device__ Encoder::Encoder(int m, char* temp, unsigned char* buffer_ptr, size_t tsz, size_t itr) :
    mode(m), inout(temp), buffer(buffer_ptr), total_size(tsz), iterator_size(itr), x1(0), x2(0xffffffff), x(0),
...
void compress(char* destination_file, char* source_file) {
    ...
    for (int call_count = 0; call_count < device_call_count; call_count++) {
        ...
        cudaMemcpy(d_input, temp_d_input, num_of_current_thread * sizeof(char*), cudaMemcpyHostToDevice);

        cudaMemcpy(d_output, temp_d_output, num_of_current_thread * sizeof(char*), cudaMemcpyHostToDevice);

        cudaMemcpy(d_encoder_buffer, encoder_buffer, num_of_current_thread * sizeof(unsigned char*),
        ...
        // auto kernel_duration = std::chrono::duration_cast<std::chrono::milliseconds>(kernel_end_time -
        // kernel_start_time); device instance deletion
...
void decompress(const char* destination_file, const char* source_file) {
    ...
    if (mode == 's') {
        ...
        for (int call_count = 0; call_count < device_call_count; call_count++) {
            ...
            cudaMemcpy(d_input_size, input_size.data(), num_of_current_thread * sizeof(size_t), cudaMemcpyHostToDevice);
            cudaMemcpy(d_output_size, uncompressed_size.data(), num_of_current_thread * sizeof(size_t),
                       cudaMemcpyHostToDevice);
            ...
            cudaDeviceSynchronize();
            // auto init_end_time = std::chrono::high_resolution_clock::now();
            // auto init_duration = std::chrono::duration_cast<std::chrono::milliseconds>(init_end_time -
            // init_start_time); std::cout << "Device initialization time: " << init_duration.count() << " ms" << endl;
            ////////////////////paq9_cuda call (warp-cooperative geometry)///////////////////////////

            int blocks, threads;
            computeWarpLaunchGeometry(num_of_current_thread, blocks, threads);
            // auto kernel_start_time = std::chrono::high_resolution_clock::now();
            paq9_cuda<<<blocks, threads>>>(d_input_size, d_input, d_output_size, d_output, encoder_buffer,
                                           num_of_current_thread, DECOMPRESS, memory_level);

            cudaDeviceSynchronize();
            // auto kernel_end_time = std::chrono::high_resolution_clock::now();
            // auto kernel_duration = std::chrono::duration_cast<std::chrono::milliseconds>(kernel_end_time -
            // kernel_start_time); std::cout << "Kernel execution time: " << kernel_duration.count() << " ms" << endl;
            freeDeviceObjects<<<blocks, threads>>>(num_of_current_thread);
