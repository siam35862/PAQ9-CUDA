...
// Initialize
__device__ Predictor::Predictor(U8 *context1_ptr, StateMap *statemap1[N], Mix *mix1[N - 1], APM *apm1, APM *apm2, APM *apm3, HashTable<16> *hashtable_ptr)
    : c0(0), context1(context1_ptr), nibble(1), bcount(0),
      apm1(apm1), apm2(apm2), apm3(apm3), hashtable(hashtable_ptr), lzp_ptr(lzp[get_tid()])
{
...
__device__ Encoder<M>::Encoder(char *temp, unsigned char *buffer_ptr, size_t tsz, size_t itr)
    : inout(temp), total_size(tsz), x1(0), x2(0xffffffff), x(0),
      buffer(buffer_ptr), usize(0), csize(0), usum(0), csum(0),
