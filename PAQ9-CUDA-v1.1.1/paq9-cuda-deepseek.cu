...
// All assert() checks below are pure invariant checks (no side effects).
// Compiling them out removes hundreds of per-bit branches from the device
// hot loops without changing any computed values.
#define NDEBUG
#include <assert.h>
// Make sure asserts are gone even if a system header above pulled in
// <assert.h> before NDEBUG was defined.
#ifdef assert
#undef assert
#endif
...
__device__ void StateMap::update(int y, int limit) {
    ...
    U32 pt = prediction_table[cntxt]; // single load instead of 4 read-modify-writes
    int n = pt & 1023, p = pt >> 10; // count, prediction

    if (n < limit)
        ++pt;
    else
        pt = pt & 0xfffffc00 | limit;

    pt += (((y << 22) - p) >> 3) * state_map_dt[n] & 0xfffffc00;
    prediction_table[cntxt] = pt;
}
...
__device__ size_t MEM = 1 << (Base_Memory_Level + 1); // Global memory limit, 1 << Base_Memory_Level+(memory option)
__device__ inline bool isalpha_device(char ch) { return (ch >= 'A' && ch <= 'Z') || (ch >= 'a' && ch <= 'z'); }
...
__device__ LZP::LZP(StateMap* statemap, U8* buf, U32* tab, APM* apm1, APM* apm2, APM* apm3) :
    N(MEM / 8), H(MEM / 32), match(-1), len(0), pos(0), hash(0), hash1(0), hash2(0), statemap(statemap), apm1(apm1),
    apm2(apm2), apm3(apm3), literals(0), matches(0), word0(0), word1(0) {
    assert(MEM > 0);
    assert(H > 0);
    buffer = buf;
    table = tab;
...
__device__ int LZP::predict_char() { return len >= MINLEN ? buffer[match & N - 1] : -1; }
...
class Predictor
{
    ...
    LZP* lzp_ptr; // cached lzp[tid], avoids repeated global-memory loads

public:
    __device__ Predictor(U8* context1_ptr, StateMap* statemap1[N], Mix* mix1[N - 1], APM* apm1, APM* apm2, APM* apm3,
...
// Initialize
__device__ Predictor::Predictor(U8* context1_ptr, StateMap* statemap1[N], Mix* mix1[N - 1], APM* apm1, APM* apm2,
                                APM* apm3, HashTable<16>* hashtable_ptr) :
    c0(0), context1(context1_ptr), nibble(1), bcount(0), apm1(apm1), apm2(apm2), apm3(apm3), hashtable(hashtable_ptr),
    lzp_ptr(lzp[get_tid()]) {
...
__device__ int Predictor::predict_next_bit() {
    assert(lzp_ptr);
    if (c0 == 0)
        return lzp_ptr->probability();
    else {

        // Set context pointers
        int pc = lzp_ptr->predict_char(); // mispredicted byte
        int r = pc + 256 >> 8 - bcount == c0; // c0 consistent with mispredicted byte?
        U32 c4 = lzp_ptr->context4(); // last 4 whole context bytes, shifted into LSB
        U32 c8 = (lzp_ptr->context8() << 4) - 1; // hash of last 7 bytes with 4 trailing 1 bits
        if ((bcount & 3) == 0) { // nibble boundary?  Update context pointers
            ...
            cp[9] = hashtable->operator[]((lzp_ptr->word0 * 5 + c0 + pcr * 17));
            cp[10] = hashtable->operator[]((lzp_ptr->word1 * 7 + lzp_ptr->word0 * 11 + c0 + pcr * 37));
        }
        ...
        for (int i = 1; i < N; ++i) {
            ...
            int st = *sp[i];
            pr = mix[i - 1]->prediction(pr, stretch->operator()(statemap[i]->predict_next_bit(st)), st + r) * 3 + pr >>
                2;
        }
...
private:
    ...
    Predictor* pred; // cached predictor[tid]
    const int tid; // cached thread id

public:
    size_t iterator_size;
    __device__ Encoder(char* temp, unsigned char* buffer_ptr, size_t tsz, size_t itr);
...
template <int M>
__device__ Encoder<M>::Encoder(char* temp, unsigned char* buffer_ptr, size_t tsz, size_t itr) :
    inout(temp), total_size(tsz), x1(0), x2(0xffffffff), x(0), buffer(buffer_ptr), usize(0), csize(0), usum(0), csum(0),
    iterator_size(itr), pred(predictor[get_tid()]), tid(get_tid()) {
...
template <int M>
__device__ bool Encoder<M>::put4(U32 c) {
...
template <int M>
__device__ bool Encoder<M>::flush() {
    if (mode == COMPRESS) {
        buffer[csize++] = x1 >> 24;
        buffer[csize++] = 255;
        buffer[csize++] = 255;
...
__global__ void paq9_cuda(size_t* input_size, char** input, size_t* output_size, char** output, unsigned char** buffer,
    ...
    if (mode == COMPRESS) {
        ...
        int itr = 0;
        Encoder<COMPRESS> encoder(output[tid], buffer[tid], input_size[tid], itr);
        int ch;
        LZP* lz = lzp[tid];
        size_t n_bytes = input_size[tid];
        ...
        if (store_mode) {
            ...
            memcpy(output[tid] + encoder.iterator_size, input[tid], n_bytes);
            encoder.iterator_size += n_bytes;
        }
        ...
        if (input[tid][0] == '1') {
            memcpy(output[tid], input[tid] + 1, input_size[tid] - 1);
            output_size[tid] = input_size[tid] - 1;
        }
        else {
            size_t usize;
            itr2 = 0;
            ...
            while (itr < input_size[tid]) {
                ...
                Encoder<DECOMPRESS> encoder(input[tid], buffer[tid], input_size[tid], itr);
...
__global__ void init(int thread_count, ThreadBuffers* buffers, int memory_level) {
    ...
    if (tid < thread_count) {
        ...
        predictor[tid] = new Predictor(buffer.predictor_context1, predictor_statemap, predictor_mix, predictor_apm1,
                                       predictor_apm2, predictor_apm3, predictor_hashtable);
    }
}
...
void memoryAllocationForThread(int thread_count) {
    ...
    for (int i = 0; i < thread_count; i++) {
        ...
        cudaMallocTracked(&buffers[i].predictor_hashtable, (MEM_host / 2 + 128) * sizeof(U8));
...
void compress(char* destination_file, char* source_file) {
    ...
    // std::cout << chunk_MB << " " << memory_level << " " << memory_per_thread << " " << maximum_thread_per_device_call
    // << " " << chunk_B << endl;
    ...
    // --------------------------------------------------
    // Copy DEVICE POINTER ARRAYS to DEVICE (once; they are
    // identical for every device call)
    // --------------------------------------------------
    cudaMemcpy(d_input, temp_d_input, num_of_thread * sizeof(char*), cudaMemcpyHostToDevice);

    cudaMemcpy(d_output, temp_d_output, num_of_thread * sizeof(char*), cudaMemcpyHostToDevice);

    cudaMemcpy(d_encoder_buffer, encoder_buffer, num_of_thread * sizeof(unsigned char*), cudaMemcpyHostToDevice);
    ...
    for (int call_count = 0; call_count < device_call_count; call_count++) {
        ...
        int num_of_current_thread =
            std::min(maximum_thread_per_device_call, (num_of_chunks - call_count * maximum_thread_per_device_call));

        std::vector<size_t> input_size(num_of_current_thread);

        for (size_t i = 0; i < num_of_current_thread; i++) {
            size_t current_B = min(chunk_B, total_B - ((call_count * maximum_thread_per_device_call) + i) * chunk_B);

            if (src_file[i] == nullptr)
                src_file[i] = new char[chunk_B];
        ...
        cudaMemcpy(d_input_size, input_size.data(), num_of_current_thread * sizeof(size_t), cudaMemcpyHostToDevice);
        ...
        for (int i = 0; i < num_of_current_thread; i++) {
            ...
            cudaMemcpy(temp_d_input[i], src_file[i], input_size[i] * sizeof(char), cudaMemcpyHostToDevice);
        }
        ...
        err1 = cudaGetLastError();
        if (err1 != cudaSuccess) {
            std::cerr << "Launch error during initialization: " << cudaGetErrorString(err1) << '\n';
            exit(1);
        }

        err1 = cudaDeviceSynchronize();
        if (err1 != cudaSuccess) {
            std::cerr << "Kernel error during initialization: " << cudaGetErrorString(err1) << '\n';
            exit(1);
        }
        ...
        paq9_cuda<<<blocks, threads>>>(d_input_size, d_input, d_output_size, d_output, d_encoder_buffer,
                                       num_of_current_thread, COMPRESS, memory_level);

        cudaDeviceSynchronize();
        auto kernel_end_time = std::chrono::high_resolution_clock::now();
        auto kernel_duration =
            std::chrono::duration_cast<std::chrono::milliseconds>(kernel_end_time - kernel_start_time);
        ...
        for (int i = 0; i < num_of_current_thread; i++) {
            ...
            cudaMemcpy(output[i], temp_d_output[i], output_size[i] * sizeof(char), cudaMemcpyDeviceToHost);
        }
...
void decompress(const char* destination_file, const char* source_file) {
    ...
    if (mode == 's') {
        ...
        if (maximum_thread_per_device_call == 0) {
            std::cout
                << "Your system does not have enough memory for running this compression/decompression algorithm.\n";
            exit(1);
        }
        int device_call_count = (num_of_chunks + maximum_thread_per_device_call - 1) / maximum_thread_per_device_call;
        // size_t chunk_B = chunk_MB * MB;
        ...
        for (int i = 0; i < num_of_thread; i++) {
            ...
            // Currently output size == input size
            // because your kernel only copies data.
            cudaMallocTracked(&temp_d_output[i], (chunk_B) * sizeof(char));
        }
        ...
        for (int call_count = 0; call_count < device_call_count; call_count++) {
            ...
            cudaMemcpy(d_input_size, input_size.data(), num_of_current_thread * sizeof(size_t), cudaMemcpyHostToDevice);
            for (int i = 0; i < num_of_current_thread; i++) {
                cudaMemcpy(temp_d_input[i], input[i], input_size[i] * sizeof(char), cudaMemcpyHostToDevice);
            }
            ...
            for (size_t i = 0; i < num_of_current_thread; i++) {
                ...
                total_input += input_size[i];
                total_output += output_size[i];
                // std::cout << "Thread: " << i + 1 << " " << input_size[i] << " Byte -> " << output_size[i] << " Byte"
                // << endl;
            }
            ...
            // std::cout << "Device Call: " << call_count + 1 << " Success\n";
        }
...
int main(int argc, char** args) {
    ...
    std::cout << "Total GPU memory allocated: " << total_cuda_malloc_allocated << " bytes ("
              << static_cast<double>(total_cuda_malloc_allocated) / (1024 * 1024) << " MiB)" << endl;
