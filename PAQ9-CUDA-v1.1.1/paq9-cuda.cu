...
void finish_device_call_memory() {
    ...
    cudaMemcpyFromSymbol(&device_heap_allocated, device_call_heap_allocated, sizeof(device_heap_allocated));
    unsigned long long device_call_memory = current_cuda_malloc_allocated + device_heap_allocated;
...
public:
    ...
    __device__ void alloc(T*& p, size_t allocate_size) {
        ...
        atomicAdd(&device_call_heap_allocated, static_cast<unsigned long long>(allocate_size) * sizeof(T));
    }
};
...
__device__ LZP::LZP() :
    N(MEM / 8), H(MEM / 32), match(-1), len(0), pos(0), hash(0), hash1(0), hash2(0), statemap1(0x200), apm1(0x10000),
    apm2(0x40000), apm3(0x100000), literals(0), matches(0), word0(0), word1(0) {
    assert(MEM > 0);
    assert(H > 0);
...
__device__ int LZP::predict_char() { return len >= MINLEN ? buf[match & N - 1] : -1; }
...
__device__ Predictor::Predictor() :
    c0(0), nibble(1), bcount(0), hashtable(MEM / 2), apm1(0x10000), apm2(0x10000), apm3(0x10000) {
...
__device__ Encoder::Encoder(int m, char* temp, size_t tsz, size_t itr) :
    mode(m), inout(temp), total_size(tsz), iterator_size(itr), x1(0), x2(0xffffffff), x(0), usize(0), csize(0), usum(0),
    csum(0) {
    int tid = get_tid();
    buf = 0;
...
__global__ void paq9_cuda(size_t* input_size, char** input, size_t* output_size, char** output, int num_of_chunks,
                          int mode, int memory_level) {
    int tid = get_tid();
...
size_t getMaximumHeapLimit() {
    ...
    if (err != cudaSuccess) {
        std::cerr << "Failed to get CUDA device properties: " << cudaGetErrorString(err) << '\n';
        exit(1);
    }
...
void compress(char* destination_file, char* source_file) {
    ...
    for (int call_count = 0; call_count < device_call_count; call_count++) {
        ...
        paq9_cuda<<<blocks, threads>>>(d_input_size, d_input, d_output_size, d_output, num_of_current_thread, COMPRESS,
                                       memory_level);
    ...
    if (mode == 's') {
        ...
        for (int call_count = 0; call_count < device_call_count; call_count++) {
            ...
            for (int i = 0; i < num_of_current_thread; i++) {
                ...
                // Currently output size == input size
                // because your kernel only copies data.
                cudaMallocTracked(&temp_d_output[i], (uncompressed_size[i]) * sizeof(char));
            ...
            paq9_cuda<<<blocks, threads>>>(d_input_size, d_input, d_output_size, d_output, num_of_current_thread,
...
int main(int argc, char** args) {
    ...
    std::cout << "Total GPU memory allocated: " << maximum_device_call_memory << " bytes ("
              << static_cast<double>(maximum_device_call_memory) / (1024 * 1024) << " MiB)" << endl;
    std::cout << "  cudaMalloc: " << maximum_cuda_malloc_for_device_call
              << " bytes; device heap: " << maximum_device_heap_for_device_call << " bytes" << endl;
