...
int Ilog::operator()(U16 x) const { return table[x]; }
...
template <int B>
HashTable<B>::HashTable(int n, U8 *table_ptr)
...
template <int B> U8 *HashTable<B>::operator[](U32 i) {
  i *= 123456791;
  i = i << 16 | i >> 16;
  i *= 234567891;
  int chk = i >> 24;
...
public:
  ...
  LZP(size_t MEM, StateMap *statemap1, U8 *buffer, U32 *table, APM *apm1,
      APM *apm2, APM *apm3);
  int predict_char();
  int context(int i);
...
LZP::LZP(size_t MEM, StateMap *statemap, U8 *buf, U32 *tab, APM *apm1,
         APM *apm2, APM *apm3)
...
public:
  Predictor(U8 *context1_ptr, StateMap *statemap1[N], Mix *mix1[N - 1],
            APM *apm1, APM *apm2, APM *apm3, HashTable<16> *hashtable_ptr,
            LZP *lzp);
...
Predictor::Predictor(U8 *context1_ptr, StateMap *statemap1[N], Mix *mix1[N - 1],
                     APM *apm1, APM *apm2, APM *apm3,
                     HashTable<16> *hashtable_ptr, LZP *lzp)
    : c0(0), nibble(1), bcount(0), hashtable(hashtable_ptr), apm1(apm1),
      apm2(apm2), apm3(apm3), context1(context1_ptr), lzp(lzp) {
  for (int i = 0; i < N; ++i) {
    sp[i] = cp[i] = context1;
...
int Predictor::predict_next_bit() {
  ...
  if ((bcount & 3) == 0) { // nibble boundary? update context pointers
    ...
    cp[10] = hashtable->operator[](
        (lzp->word1 * 7 + lzp->word0 * 11 + c0 + pcr * 37));
  }
...
public:
  ...
  Encoder(int m, char *temp, unsigned char *buffer_ptr, Predictor *predictor,
...
Encoder::Encoder(int m, char *temp, unsigned char *buffer_ptr,
                 Predictor *predictor, size_t tsz, size_t itr)
    : mode(m), inout(temp), total_size(tsz), predictor(predictor), x1(0),
      x2(0xffffffff), x(0), buffer(buffer_ptr), usize(0), csize(0),
      iterator_size(itr) {
...
static void buildChunkModel(ChunkBuffers &cb, size_t chunkMEM, ChunkModel &m) {
  ...
  m.lzp = std::make_unique<LZP>(chunkMEM, m.lzpSM.get(), cb.lzp_buffer.data(),
                                cb.lzp_table.data(), m.lzpApm1.get(),
                                m.lzpApm2.get(), m.lzpApm3.get());

  StateMap *psm[11];
  for (int j = 0; j < 11; ++j) {
    m.smOwn.push_back(
        std::make_unique<StateMap>(cb.predictor_statemap[j].data(), 0x100));
    psm[j] = m.smOwn.back().get();
  }
  Mix *pmix[10];
  for (int j = 0; j < 10; ++j) {
    m.mixOwn.push_back(
        std::make_unique<Mix>(cb.predictor_mix[j].data(), 0x400));
  ...
  m.hashtable = std::make_unique<HashTable<16>>((int)(chunkMEM / 2),
                                                cb.predictor_hashtable.data());

  m.predictor = std::make_unique<Predictor>(
      cb.predictor_context1.data(), psm, pmix, m.predApm1.get(),
      m.predApm2.get(), m.predApm3.get(), m.hashtable.get(), m.lzp.get());
}

static size_t chunkMEM(int mem_level) { return (size_t)1 << (19 + mem_level); }
...
static void compressChunk(const char *input, size_t inputSize, int mem_level,
  ...
  Encoder encoder(COMPRESS, output.data(), cb.encoder_buffer.data(), &predictor,
                  inputSize, 0);
...
static void decompressChunk(const char *input, size_t inputSize, int mem_level,
  ...
  while (itr < inputSize) {
    ...
    Encoder encoder(DECOMPRESS, inMutable, cb.encoder_buffer.data(), &predictor,
                    inputSize, itr);
...
void compress(const char *destination_file, const char *source_file) {
  ...
  int num_of_chunks =
      total_B == 0 ? 0 : (int)((total_B + chunk_B - 1) / chunk_B);
  ...
  std::cout << "CPU worker threads: " << numThreads << " for " << num_of_chunks
            << " chunks" << endl;
  ...
  auto worker = [&]() {
    while (true) {
      ...
      compressChunk(fileData.data() + offset, curSize, memory_level,
                    outputs[idx], outputSizes[idx]);
    }
  };
...
int main(int argc, char **args) {
  ...
  if (seconds > 0)
    std::cout << "Compression/Decompression Speed: "
              << (mode == COMPRESS ? total_uncompressed_size
